...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template match="/">
        <html>
            <head>
                ...
                <script>   
                       function showVerovio(){
                           const notationVerovioElems = document.querySelectorAll('.notationVerovio');
                           const imageElems = document.querySelectorAll('.notationImg');
                           for (elem of notationVerovioElems) {elem.classList.remove('hidden')}       
                           for (elem of imageElems) {elem.classList.add('hidden')}
                        }
                        
                        function showImages(){
                            const notationVerovioElems = document.querySelectorAll('.notationVerovio');
                            const imageElems = document.querySelectorAll('.notationImg');
                            for (elem of imageElems) {elem.classList.remove('hidden')}
                            for (elem of notationVerovioElems) {elem.classList.add('hidden')}                  
                        }
            ...
            <body>
                ...
                <button onclick="showVerovio()">Verovio</button>
    ...
    <xsl:template match="tei:notatedMusic">
        ...
        <img src="{$imgURL}" class="notationImg hidden"/>
